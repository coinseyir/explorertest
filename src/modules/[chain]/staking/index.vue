...
<template>
<div>
    <div class="bg-white dark:bg-gray-800 rounded-lg grid sm:grid-cols-1 md:grid-cols-4 p-4 border border-gray-200 dark:border-gray-700">    
        <div class="flex">
            <span>
                <div class="relative w-9 h-9 rounded overflow-hidden flex items-center justify-center mr-2">
                    <Icon class="text-green-500" icon="mdi:trending-up" size="32" />
                    <div class="absolute top-0 left-0 bottom-0 right-0 opacity-20 bg-green-500"></div>
                </div>
            </span>
            <span>
                <div class="font-bold text-gray-800 dark:text-gray-100">{{ format.percent(mintStore.inflation) }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-300">{{ $t('staking.inflation') }}</div>
            </span>
        </div>
        <div class="flex">
            <span>
                <div class="relative w-9 h-9 rounded overflow-hidden flex items-center justify-center mr-2">
                    <Icon class="text-indigo-500" icon="mdi:lock-open-outline" size="32" />
                    <div class="absolute top-0 left-0 bottom-0 right-0 opacity-20 bg-indigo-500"></div>
                </div>
            </span>
            <span>
                <div class="font-bold text-gray-800 dark:text-gray-100">{{ formatSeconds(staking.params?.unbonding_time) }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-300">{{ $t('staking.unbonding_time') }}</div>
            </span>
        </div> 
        <div class="flex">
            <span>
                <div class="relative w-9 h-9 rounded overflow-hidden flex items-center justify-center mr-2">
                    <Icon class="text-red-500" icon="mdi:alert-octagon-outline" size="32" />
                    <div class="absolute top-0 left-0 bottom-0 right-0 opacity-20 bg-red-500"></div>
                </div>
            </span>
            <span>
            <div class="font-bold text-gray-800 dark:text-gray-100">{{ format.percent(slashing.slash_fraction_double_sign) }}</div>
            <div class="text-xs text-gray-600 dark:text-gray-300">{{ $t('staking.double_sign_slashing') }}</div>
            </span>
        </div> 
        <div class="flex">
            <span>
                <div class="relative w-9 h-9 rounded overflow-hidden flex items-center justify-center mr-2">
                    <Icon class="text-red-500" icon="mdi:pause" size="32" />
                    <div class="absolute top-0 left-0 bottom-0 right-0 opacity-20 bg-red-500"></div>
                </div>
            </span>
            <span>
            <div class="font-bold text-gray-800 dark:text-gray-100">{{ format.percent(slashing.slash_fraction_downtime) }}</div>
            <div class="text-xs text-gray-600 dark:text-gray-300">{{ $t('staking.downtime_slashing') }}</div>
            </span>
        </div>  
    </div>
...
<div>
    <div class="flex items-center justify-between py-1">
        <div class="tabs tabs-boxed bg-transparent">
            <a
                class="tab text-gray-600 dark:text-gray-300"
    ...
    <div class="bg-white dark:bg-gray-800 px-4 pt-3 pb-4 rounded shadow border border-gray-200 dark:border-gray-700">
                    ...
                    >
                        ...
                        <td>
                            <div
                                ...
                                :class="`text-${rank === 'error' ? 'red-500' : rank === 'warning' ? 'yellow-500' : 'indigo-500'}`"
                            >
                                <span
                                    class="inset-x-0 inset-y-0 opacity-10 absolute"
                                    :class="`bg-${rank === 'error' ? 'red-500' : rank === 'warning' ? 'yellow-500' : 'indigo-500'}`"
                                ></span>
                                {{ i + 1 }}
                            </div>
                        </td>
                        ...
                        <td>
                            ...
                            >
                                ...
                                <div class="flex flex-col">
                                    <span class="text-sm text-indigo-500 dark:invert whitespace-nowrap overflow-hidden">
                                        <RouterLink
                                            :to="{
                                    ...
                                    <span class="text-xs text-gray-600 dark:text-gray-300">{{ v.description?.website ||
                        ...
                        <td class="text-right">
                            <div class="flex flex-col">
                                <h6 class="text-sm font-weight-medium whitespace-nowrap text-gray-800 dark:text-gray-100">
                                    {{
                                        format.formatToken(
                                            {
                                                amount: parseInt(
                        ...
                        <!-- 👉 commission -->
                        <td class="text-right text-xs text-gray-600 dark:text-gray-300">
                            {{
                        ...
                        <td class="text-center">
                            ...
                            <label
                                v-else
                                for="delegate"
                                class="btn btn-xs btn-indigo-500 rounded-sm capitalize"
                                @click="
        ...
        <div class="flex flex-row items-center">
            <div
                class="text-xs truncate relative py-2 px-4 rounded-md w-fit text-red-500 mr-2"
            >
                <span
                    class="inset-x-0 inset-y-0 opacity-10 absolute bg-red-500"
                ></span>
                {{ $t('staking.top') }} 33%
            </div>
            <div
                class="text-xs truncate relative py-2 px-4 rounded-md w-fit text-yellow-500"
            >
                <span
                    class="inset-x-0 inset-y-0 opacity-10 absolute bg-yellow-500"
                ></span>
                {{ $t('staking.top') }} 67%
            </div>
            <div class="text-xs text-gray-600 dark:text-gray-300 hidden md:!block pl-2">
